...
```output
PASS: 79 relay, isolation, recovery and independent-worker checks
```
...
- **the data is real.** It is a synthetic fixture, generated by
  `examples/mimic_fixture.py`, and every reply says so. Nothing here has been
  run against credentialed MIMIC-IV.
